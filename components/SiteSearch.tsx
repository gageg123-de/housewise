...
export default function SiteSearch({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  return <div className="search-page"><label htmlFor="library-search">Search titles, symptoms, systems, and keywords</label><input id="library-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try “musty garage” or “toilet bubbling”"/><p aria-live="polite">{results.length} {results.length === 1 ? "guide" : "guides"}</p><div className="result-list">{results.map((article) => <Link key={article.slug} href={articleUrl(article)}><strong>{article.title}</strong><span>{titleCase(article.primary_category)} · {article.description}</span></Link>)}</div></div>;
}
