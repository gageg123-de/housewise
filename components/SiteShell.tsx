...
export function Footer() {
  return <footer className="footer"><div className="footer-grid"><div><BrandWordmark footer/><p>{tagline}</p></div><div><h2>Explore</h2><Link href="/find-a-problem/">Problem Finder</Link><Link href="/search/">Search guides</Link><Link href="/sitemap.xml">Sitemap</Link></div><div><h2>Categories</h2>{categories.slice(0, 5).map((category) => <Link key={category.slug} href={`/${category.slug}/`}>{category.name}</Link>)}</div><div><h2>About</h2><Link href="/about/">About</Link><Link href="/editorial-policy/">Editorial policy</Link><Link href="/contact/">Contact</Link><Link href="/privacy/">Privacy</Link><Link href="/terms/">Terms & disclaimer</Link></div></div><p className="fine-print">© {new Date().getFullYear()} {brandName}. General information only; not a substitute for professional evaluation.</p></footer>;
}
