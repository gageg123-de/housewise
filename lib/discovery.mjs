...
const locationDefinitions = [
  { value: "bathroom", label: "Bathroom", locations: ["bathroom"], systems: ["plumbing", "bathroom", "moisture-and-mold"], symptoms: [
    ...
    { value: "bath-water-sediment", label: "Sediment or particles in bath water", articleSymptoms: ["discoloration", "plumbing-behavior"], preferredSlugs: ["sediment-in-bath-water"] },
    { value: "window-condensation", label: "Window glass is wet or foggy inside", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["condensation-inside-windows"] },
    { value: "window-between-panes", label: "Fog or moisture is trapped between window panes", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["condensation-between-window-panes"] },
    { value: "window-rain", label: "Window leaks during or after rain", articleSymptoms: ["moisture", "leaking", "discoloration"], preferredSlugs: ["window-leaking-when-it-rains"] },
    { value: "window-mold", label: "Mold-like growth around a window", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["mold-growing-around-windows"] },
    { value: "ceiling-mold", label: "Mold-like growth on the ceiling", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["mold-growing-on-ceiling"] },
    { value: "ceiling-stain", label: "Brown or yellow water stain on the ceiling", articleSymptoms: ["moisture", "discoloration", "leaking"], preferredSlugs: ["water-stain-on-ceiling"] },
    { value: "ceiling-rain", label: "Ceiling gets wet after rain", articleSymptoms: ["moisture", "leaking", "discoloration"], preferredSlugs: ["ceiling-wet-after-rain"] },
    { value: "walls-sweating", label: "Walls are wet or sweating", articleSymptoms: ["moisture"], preferredSlugs: ["walls-sweating"] },
    { value: "wall-wet-spot", label: "One localized wet or damp spot on a wall", articleSymptoms: ["moisture", "leaking", "discoloration"], preferredSlugs: ["wet-spot-on-wall"] },
    { value: "wall-rain", label: "Wall gets wet during or after rain", articleSymptoms: ["moisture", "leaking", "discoloration"], preferredSlugs: ["wall-wet-after-rain"] },
    { value: "sink-drain-leak", label: "Sink drain assembly is leaking", articleSymptoms: ["leaking", "drainage"], preferredSlugs: ["sink-leaking-from-drain"] },
  ...
  { value: "bedroom", label: "Bedroom", locations: ["bedroom", "living-area"], systems: ["hvac", "electrical", "windows-and-doors"], symptoms: [
    ...
    { value: "bedroom-musty-smell", label: "Bedroom smells musty or damp", articleSymptoms: ["smell", "moisture"], preferredSlugs: ["bedroom-smells-musty"] },
  ...
  { value: "laundry", label: "Laundry area", locations: ["laundry"], systems: ["appliances", "plumbing", "electrical"], symptoms: [
    ...
    { value: "dryer-shuts-off", label: "Dryer stops before the cycle finishes", articleSymptoms: ["appliance-behavior", "heat", "electrical-behavior"], preferredSlugs: ["dryer-keeps-shutting-off"] },
  ...
  { value: "attic", label: "Attic", locations: ["attic"], systems: ["attic-and-insulation", "hvac", "roofing", "pests"], symptoms: [
    ...
    { value: "attic-mold", label: "Mold-like growth on attic wood", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["mold-in-attic"] },
  ...
  { value: "whole-house", label: "Whole house", locations: ["whole-house"], systems: ["hvac", "electrical", "plumbing", "moisture-and-mold"], symptoms: [
    ...
    { value: "window-condensation", label: "Windows are wet or foggy inside", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["condensation-inside-windows"] },
    { value: "window-between-panes", label: "Fog or moisture is trapped between window panes", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["condensation-between-window-panes"] },
    { value: "window-rain", label: "A window leaks during or after rain", articleSymptoms: ["moisture", "leaking", "discoloration"], preferredSlugs: ["window-leaking-when-it-rains"] },
    { value: "window-mold", label: "Mold-like growth appears around windows", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["mold-growing-around-windows"] },
    { value: "ceiling-mold", label: "Mold-like growth appears on ceilings", articleSymptoms: ["moisture", "discoloration"], preferredSlugs: ["mold-growing-on-ceiling"] },
    { value: "ceiling-stain", label: "Brown or yellow water stain on the ceiling", articleSymptoms: ["moisture", "discoloration", "leaking"], preferredSlugs: ["water-stain-on-ceiling"] },
    { value: "ceiling-rain", label: "Ceiling gets wet after rain", articleSymptoms: ["moisture", "leaking", "discoloration"], preferredSlugs: ["ceiling-wet-after-rain"] },
    { value: "walls-sweating", label: "Walls are wet or sweating", articleSymptoms: ["moisture"], preferredSlugs: ["walls-sweating"] },
    ...
    { value: "random-light-flicker", label: "Lights flicker without an obvious trigger", articleSymptoms: ["electrical-behavior"], preferredSlugs: ["lights-flicker-randomly"] },
    { value: "smell", label: "Musty, burning, or another smell", articleSymptoms: ["smell"] },
    { value: "noise", label: "Noise or vibration", articleSymptoms: ["noise", "vibration"] },
...
export function searchArticles(articles, query) {
  ...
  return articles
    .map((article) => {
      if (article.slug === "sink-leaking-from-drain" && /\b(supply line|faucet|drains slowly|slow drain|gurgles?|dishwasher|garbage disposal)\b/.test(normalizedQuery)) return null;
      if (article.slug === "sediment-in-bath-water" && (/\b(?:coming|backing|rising)\b.*\b(?:from|up)\b.*\b(?:tub|bathtub) drain\b|\b(?:tub|bathtub) drain\b.*\b(?:clogged?|debris|dirt|sediment)\b/.test(normalizedQuery) || (!/\b(?:bath|bathtub|tub)\b/.test(normalizedQuery) && /\b(?:brown hot water|cloudy water|black specks?|white flakes?|water heater sediment|sediment in hot water|faucet water)\b/.test(normalizedQuery)))) return null;
      if (article.slug === "dryer-keeps-shutting-off" && /\b(dryer not heat(?:ing)?|dryer (?:will not|won.t) start|dryer trips? (?:the )?breaker)\b/.test(normalizedQuery)) return null;
      if (article.slug === "lights-flicker-randomly" && /\bwhen\b.*\b(appliance|ac|air conditioner|compressor|dryer|washer|microwave|refrigerator|heater|motor|pump)\b/.test(normalizedQuery)) return null;
      if (article.slug === "condensation-inside-windows" && /\bbetween (?:the )?(?:window )?panes?\b|\bwindow.*\b(?:leak(?:ing|s)?|rain)\b|\bmold\b|\bhouse humid\b|\bhumid with (?:the )?ac\b|\bac vent\b/.test(normalizedQuery)) return null;
      if (article.slug === "condensation-between-window-panes" && !/\b(?:between (?:the )?(?:window )?panes?|double pane|dual pane|insulated glass|window seal|inside (?:the )?window glass|inside (?:a )?double pane|cloudy between|haze between|water between)\b/.test(normalizedQuery)) return null;
      if (article.slug === "bedroom-smells-musty" && /\bgarage\b|\b(?:closet|carpet|house)\b.*\b(?:musty|mildew)\b|\bwhen\b.*\b(?:ac|air conditioner|hvac|heat)\b/.test(normalizedQuery) && !/\bbedroom\b/.test(normalizedQuery)) return null;
      if (article.slug === "mold-growing-around-windows" && !/\b(?:mold|moldy|growth|black stuff)\b/.test(normalizedQuery)) return null;
      if (article.slug === "mold-growing-on-ceiling" && /\b(?:water stain on (?:the )?ceiling|ceiling (?:is )?wet after rain|mold around windows?|condensation inside windows?|water dripping from (?:the )?ac vent|house humid with (?:the )?ac running)\b/.test(normalizedQuery)) return null;
      if (article.slug === "mold-in-attic" && /\b(?:mold (?:growing )?on (?:the )?ceiling|ceiling mold|water stain on (?:the )?ceiling|ceiling (?:is )?wet after rain|attic (?:is )?damp|condensation in (?:the )?attic|frost in (?:the )?attic|ductwork sweating in (?:the )?attic|water around (?:the )?air handler|why is my roof leaking)\b/.test(normalizedQuery)) return null;
      if (article.slug === "water-stain-on-ceiling" && /\b(?:mold (?:growing )?on (?:the )?ceiling|ceiling mold|ceiling (?:stain|is wet) after rain|water dripping from (?:the )?ceiling|ceiling (?:is )?(?:sagging|bulging)|mold in (?:the )?attic|water dripping from (?:the )?ac vent|water around (?:the )?indoor ac unit)\b/.test(normalizedQuery)) return null;
      if (article.slug === "ceiling-wet-after-rain" && !/\b(?:rain|rains|raining|rainfall|storm|wind driven)\b/.test(normalizedQuery)) return null;
      if (article.slug === "walls-sweating" && /\b(?:wet spot on (?:the )?wall|wall (?:is )?wet after rain|mold (?:growing )?on (?:the )?wall|mold around windows?|condensation inside windows?|ceiling mold|mold (?:growing )?on (?:the )?ceiling|water dripping from (?:the )?ac vent|house humid with (?:the )?ac running)\b/.test(normalizedQuery)) return null;
      if (article.slug === "wet-spot-on-wall" && /\b(?:walls? sweating|condensation on walls?|walls? wet from condensation|walls? wet after rain|mold (?:growing )?on (?:the )?wall|moldy wall|paint (?:is )?(?:bubbling|blistering|peeling)|bubbling paint|soft drywall|drywall (?:is )?soft|window (?:is )?leaking when it rains|mold around windows?|condensation inside windows?|water stain on (?:the )?ceiling|ceiling (?:is )?wet after rain)\b/.test(normalizedQuery)) return null;
      if (article.slug === "wall-wet-after-rain" && (!/\b(?:rain|rains|raining|rainfall|storm|wind driven)\b/.test(normalizedQuery) || /\b(?:window|ceiling)\b/.test(normalizedQuery) && !/\bwall\b/.test(normalizedQuery))) return null;
      if (article.slug === "window-leaking-when-it-rains" && (!/\b(?:rain|rains|raining|rainfall|storm|wind driven)\b/.test(normalizedQuery) || !/\b(?:window|frame)\b/.test(normalizedQuery))) return null;
